...
//! ```
//!
//! Note how efficient the encoding becomes for repetetive data structures: the last `species =
//! #PrionailurusViverrinus` only requires two bytes on wire!
//!
//! For comparison, `serde_json` produces a string of 210 bytes for the given input while msgpack
//! in self-describing mode still needs 176 bytes.
